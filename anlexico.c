...
int cont_esp=0;    //variabe para contar la cantidad de espacio
...
void sigLex()
{
	...
	while((c=fgetc(archivo))!=EOF)
	{
		...
		if (c==' '){
		    do{
                        if(c==' ')
                            cont_esp = cont_esp + 1;//contador de espacios para luego imprimir
                            c=fgetc(archivo); 
                    }while(c <> ' ');//ver si es un " HASTA QUE ENCUENTRE " O CARACTERO \N
        ...
        { 
            ...
            while(c=='\t'){ 
                agregar_espacio(); //funcion que agregara la cantidad de espacios de acuerdo a la cantidad de tabulaciones corridas
		c=fgetc(archivo);
            }
        }
        else if(c=='\n')
		...
		{
			//es un attribute-name o un attribute-value
			i=0;
			do{
				id[i]=c;
				i++;
